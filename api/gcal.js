...
const getCalURL = cal => cal in CALENDARS ? url`https://www.googleapis.com/calendar/v3/calendars/${CALENDARS[cal]}/events?key=${process.env.GOOGLE_CALENDAR_API_KEY}` : null;

async function getCal(cal = 'pantry', { signal } = {}) {
	const url = getCalURL(cal?.toLowerCase());
