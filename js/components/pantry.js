...
export const getHouseholdSize = household => `<input type="number" name="household" id="${HOUSEHOLD_SIZE_ID}" class="input" placeholder="## *" min="1" max="${MAX_HOUSEHOLD_SIZE}" inputmode="numeric" autocomplete="off" ${attr({ value: household })} required="" />`;
...
export const getPantryHouseholdTemplate = ({ signal }) => `<template id="${HOUSEHOLD_TEMPLATE_ID}">
	<li class="flex row ${HOUSEHOLD_MEMBER_CLASSNAME}">
		<input type="text" name="person[]" required="" class="household-member input" minlength="3" ${onChange}="${pantryHouseholdSizeChange}" ${signalAttr}="${signal}">
		<input type="date" name="bDay[]" class="household-member input" required="" />
